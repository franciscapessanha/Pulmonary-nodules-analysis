...
def getPrediction(train_features, train_y, val_features, val_y):
    modelSVM = SVC(kernel = "linear", gamma = 'auto', class_weight = "balanced")
...
print("\nShape Features only \n=======================")
prediction_int = getPrediction(train_shape ,train_y, val_shape , val_y)

print("\nLBP Features only \n=======================")
prediction_lbp = getPrediction(train_lbp, train_y, val_lbp, val_y)

print("\nAll Features\n=======================")
train_features = np.concatenate((train_int, train_circ, train_shape, train_lbp), axis=1)
val_features = np.concatenate((val_int, val_circ, val_shape,  val_lbp), axis=1)
test_features = np.concatenate((test_int, test_circ, test_shape,test_lbp), axis=1)
